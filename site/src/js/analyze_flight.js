...
function prepare(fixes) {
    for (let i = 0; i < fixes.length; i++) {
        ...
        if (i > 0) {
            ...
            if (deltaTimestamp > 0) {
                fixes[i].dist = (new Point(fixes, i - 1).distanceEarth(new Point(fixes, i)))
                fixes[i].hspeed = fixes[i].dist * 1000 /
                    deltaTimestamp * 1000;
...
function detectLaunchLanding(fixes) {
    let ll = [];
    for (let i = 0; i < fixes.length - 1; i++) {
        // Assume the fix is on the ground
        if (fixes[i].stateFlight) {
            // Iterate back until we know we're on the ground
            let j;
            for (j = i; j > 0 && !fixes[j].stateGround; j--);
            const launch = j;
            // Iterate forward until we know we're on the ground again
...
export function analyze(flight, config) {
    ...
    if (config.trim || config.detectLaunch || config.detectLanding || config.analyze) {
